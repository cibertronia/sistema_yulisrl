$(document).ready(function () {
	...
	$(document).on('click', '.plusABONO', function (event) {
		...
		$(".tablaAbonoCredito").removeClass('d-none');
		$("#byCantidad_").val("");
		$("#byAnticipo").val("");
		$.ajax({
			...
			type: 'POST',
			dataType: 'json',
			data: { idCotizacion_Recibos: idCotizacion },
			success: function (data) {
				...
				$("#selectMoneda option:selected").val(data.Moneda);
	...
	$(document).on('click', '.closeTablaCredito', function (event) {
		event.preventDefault();
		$(".tableCotizaciones").removeClass('d-none');
		$(".tablaAbonoCredito").addClass('d-none');
	});
	...
	$(document).on('click', '.editarAbono', function (event) {
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (data.Moneda == 'USD') {
					$("#Cant_Abono").html("<span class='text-danger'>$</span> " + data.AbonoUSD + " <span class='text-danger'>Dólares</span>");
				} else {
					$("#Cant_Abono").html("<span class='text-danger'>Bs</span> " + data.porAbono + " <span class='text-danger'>Bolivianos</span>");
				}
			}
		})
	});
	$(document).on('click', '.closeEditCredito1', function (event) {
		event.preventDefault();
		$(".tableCotizaciones").removeClass('d-none');
		$(".formEditCredito1").addClass('d-none');
	});
	/*	LLAMA LOS DATOS DEL SEGUNDO ABONO EN ADELANTE (EL ULTIMO ABONO REALIZADO)	*/
	$(document).on('click', '.ultimoAbono', function (event) {
		event.preventDefault();
		var idRecibo = $(this).attr("id");
		$(".tableCotizaciones").addClass('d-none');
		$(".formEditCredito2").removeClass('d-none');
		$.ajax({
			url: 'includes/consultas.php',
			type: 'POST',
			dataType: 'json',
			...
			success: function (data) {
				...
				$("#select_Moneda option:selected").val(data.Moneda);
				if (data.Moneda == 'USD') {
					$("#by_Cantidad_").val(data.AbonoUSD);
					$("#by_Anticipo_").val(data.AbonoUSD);
					$("#by_Total_").val(data.TotalUSD);
				} else {
					$("#by_Cantidad_").val(data.porAbono);
					$("#by_Anticipo_").val(data.porAbono);
	...
	$(document).on('click', '.closeEditCredito2', function (event) {
		event.preventDefault();
		$(".tableCotizaciones").removeClass('d-none');
		$(".formEditCredito2").addClass('d-none');
	});
	...
	$(document).on('click', '.updatePrimerAbono', function (event) {
		...
		$.ajax({
			...
			type: 'POST',
			dataType: 'html',
			data: $("#formAbonoCredito").serialize(),
		})
			.done(function (data) {
				$(".respuesta").html(data);
			})
	...
	/*	ACTUALIZANDO LOS DATOS DEL SEGUNDO EN ADELANTE DE LOS ABONOS	*/
	...
	$(document).on('click', '.verComentarios', function (event) {
		event.preventDefault();
		var idCotizacion = $(this).attr("id");
		$.ajax({
			url: 'includes/consultas.php',
			type: 'POST',
			dataType: 'json',
			data: { idCotizaNotaE: idCotizacion },
			success: function (data) {
				$("#idNotadeEntrega").val(data.idNotaE);
				let Observa = data.Observaciones;
				$("#commentNotaE").val(Observa);
				if (Observa == '') {
					$(".updatecomment").addClass('d-none');
					$(".addcomment").removeClass('d-none');
				} else {
					$(".updatecomment").removeClass('d-none');
	...
	$(document).on('click', '.addcomment', function (event) {
		event.preventDefault();
		var idNotaE = $("#idNotadeEntrega").val();
		var Observa = $("#commentNotaE").val();
		if (Observa == '') {
			$(".noObservaciones").removeClass('d-none');
			setTimeout(function () {
				$(".noObservaciones").addClass('d-none');
				$("#commentNotaE").focus()
			}, 2500);
		} else {
			$.ajax({
				...
				type: 'POST',
				dataType: 'html',
				data: "action=AgregarObservacionesNotaE&idNotaE=" + idNotaE + "&Observaciones=" + Observa,
			})
				.done(function (data) {
					$(".respuesta").html(data);
				})
	...
	$(document).on('click', '.eliminarCreditoProceso', function (event) {
		event.preventDefault();
		var idCotizacion = $(this).attr('id');
		...
		swalWithBootstrapButtons.fire({
			title: 'Estás seguro?',
			html: "Borrar Credito en proceso",
			type: 'warning',
			...
			.then((result) => {
				if (result.value) {
					$.ajax({
						...
						type: 'POST',
						dataType: 'html',
						data: "action=eliminarCreditoProceso&idCotizacion=" + idCotizacion,
					})
						.done(function (data) {
							$(".respuesta").html(data);
						})
