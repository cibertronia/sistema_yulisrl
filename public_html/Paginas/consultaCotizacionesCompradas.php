<?php
	...
	if (isset($_POST['fechaInicio'])AND isset($_POST['fechaCierre'])) {
		$fechaInicio 	=	$_POST['fechaInicio'];
		$fechaCierre 	=	$_POST['fechaCierre']; ?>
		<!DOCTYPE html>
		<html lang="es">
			<head>
				<title>CONSULTA COTIZACIONES COMPRADAS</title>
							...
							<div class="col-md-12">
								<div class="panel panel-inverse">
									<div class="panel-heading">
										<h4 class="panel-title">COTIZACIONES COMPRADAS ENTRE EL <?php echo $fechaInicio ?> &nbsp;&nbsp;Y &nbsp;&nbsp;<?php echo $fechaCierre ?></h4>
									...
									<div class="panel-body">
										<form id="findCompras" action="?root=consultaCotizacionesCompradas" method="POST" class="d-none w-75 m-auto" data-parsley-validate="true">
											<div class="row">
												<div class="col text-center">
													<label for="fechaInicio">Buscar desde</label>
													<input type="date" name="fechaInicio" id="fechaInicio" class="form-control text-center" data-parsley-required="true">
													<div class="text-center text-danger d-none noFechaInicio">No ha seleccionado la fecha.</div>
												</div>
												<div class="col text-center">
													<label for="fechaFin">Buscar hasta</label>
													<input type="date" name="fechaCierre" id="fechaFin" class="form-control text-center" data-parsley-required="true">
													<div class="text-center text-danger d-none nofechaCierre">No ha seleccionado la fecha.</div>
												</div>
												<div class="col text-center">
													<label for="btnFind">&nbsp;&nbsp;&nbsp;</label>
													<button title="Buscar Cotizaciones vendidas en otras fechas" type="submit" class="btn btn-xs btn-danger form-control buscarVenta">BUSCAR COTIZACIONES ANTERIORES</button>
												</div>
											</div>
										</form><br>
										<table id="data-table-buttons" class="table table-striped table-bordered table-td-valign-middle w-100">
											...
											<tbody>
												<?php											
													$Num = 1;
													if ($_SESSION['Rango']=='2') {
														$queryCotiza	=	mysqli_query($MySQLi,"SELECT idCotizacion, Code, Clave, idUser, idCliente, Forma_Pago, DATE_FORMAT(FinFecha_Oferta, '%d de %M, %Y')AS FinFecha_Oferta, Dias_Entrega, Comentarios, Sucursal, DATE_FORMAT(Fecha, '%d de %M, %Y') AS Fecha, DATE_FORMAT(Hora, '%h:%i:%s %p') AS Hora, DATE_FORMAT(Compra, '%d de %M, %Y') AS Compra FROM Cotizaciones WHERE Compra BETWEEN '$fechaInicio'AND'$fechaCierre'AND Estado=2 ORDER BY Compra DESC");
														while ($dataCotiza 	=	mysqli_fetch_assoc($queryCotiza)) {
															...
															<tr class="odd gradeX">
																...
																<td style="font-size: 12px;">
																	<div class="text-center mt-1 mb-1" style="margin-top: -5%">COMPRADA EL DÍA: <span class="text-danger" style="text-transform: uppercase;"><?php echo $dataCotiza['Compra'] ?></span></div>
														...
														$queryCotiza	=	mysqli_query($MySQLi,"SELECT idCotizacion, Code, Clave, idUser, idCliente, Forma_Pago, DATE_FORMAT(FinFecha_Oferta, '%d de %M, %Y')AS FinFecha_Oferta, Dias_Entrega, Comentarios, Sucursal, DATE_FORMAT(Fecha, '%d de %M, %Y') AS Fecha, DATE_FORMAT(Hora, '%h:%i:%s %p') AS Hora, DATE_FORMAT(Compra, '%d de %M, %Y') AS Compra FROM Cotizaciones WHERE idUser='$idUser'AND Compra BETWEEN '$fechaInicio'AND'$fechaCierre'AND Estado=2 ORDER BY Compra DESC");
														while ($dataCotiza 	=	mysqli_fetch_assoc($queryCotiza)) {
															$idCotizacion 	= 	$dataCotiza['idCotizacion'];?>
															<tr class="odd gradeX">
																<td class="text-center"><?php echo $Num; ?></td>
