...
if($sucursal==''){
	...
	GROUP BY
	`idCliente`
	ORDER BY cantidad_total_comprada DESC LIMIT 10";
}else{
	$queryClientes = "SELECT
	idCliente,Sucursal,
	SUM(Cantidad) AS 'cantidad_total_comprada'
...
while ($dataClientes = mysqli_fetch_assoc($MySQLiexecute)) {
	...
	$nombreCompleto= '['.$dataClientes['cantidad_total_comprada'].'] '.$datosCliente['Nombres'].' '.$datosCliente['Apellidos'];

	array_push($datos1, $nombreCompleto);
	array_push($datos2, $dataClientes['cantidad_total_comprada']);   
}
