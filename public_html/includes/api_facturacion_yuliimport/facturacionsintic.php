...
function guardar_factura($respuesta_api, $cabeza_factura, $array_productos)
{
  ...
  $userCashier = 'User Yuli Srl'; //generar pensar bien
  ...
  $sqlInsertFactura = mysqli_query($YuliimportDB, "INSERT INTO factura (
...
'$userCashier','$siatCodeState','$siatCodeReception','$siatDescriptionStatus','$countItems','$invoiceXml','$idCotizacion','$branchId','$exceptionCode','$clientEmail','$tipoFactura')");
  ...
  foreach ($datosDecodeado as $row) {
    ...
    if ($row->prodF == 'si') {
      ...
      $prodFis = mysqli_query($YuliimportDB, "SELECT * FROM productos_fiscales WHERE idProducto='$row->idProductoFiscal'");
      $dataprodFis = mysqli_fetch_assoc($prodFis);
      $stockActual = (int) $dataprodFis['saldo_fisico'];
      $stockNuevo = $stockActual - $row->quantity;
      $updateProdFi = mysqli_query($YuliimportDB, "UPDATE productos_fiscales SET saldo_fisico='$stockNuevo' WHERE idProducto='$row->idProductoFiscal' ");
      ...
      $descripcion_historial = ($cabeza_factura["idCotizacion"] == '-1') ? "DESCUENTO STOCK-FACTURACION EMISION DIRECTA DOBLE" : "DESCUENTO STOCK-FACTURACION VENDIDAS";
    ...
    $sql = mysqli_query($YuliimportDB, "INSERT INTO detailInvoice
   (
       detailId,
       activityEconomic,
       codeProductSin,
...
function obtener_codigo_sucursal()
{
  ...
  $queryUser = mysqli_query($YuliimportDB, "SELECT Ciudad FROM Usuarios WHERE idUser='$idUser' ") or die(mysqli_error($YuliimportDB));
