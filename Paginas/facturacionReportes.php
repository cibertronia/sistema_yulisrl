...
$sucursal = $sucursalesModel->where('Sucursal', $miCiudad);
$ssucursaltxt = strtoupper($sucursal[0]['Sucursal']);
$ssucursal = $sucursal[0]['idSucursal'];
?>
<!DOCTYPE html>
<html lang="es">
    ...
    <div id="page-container" class="fade page-sidebar-fixed page-header-fixed">
        ...
        <div id="content" class="content">
            ...
            if (isset($_POST['inicio'])) {
                $Inicio     = $_POST['inicio'];
                $Fin             =    $_POST['fin'];
                $ssucursal             =    $_POST['ssucursal'];
            } else {
                $Inicio = $startBusqueda; //startbuskeda = 1 del mes
                $Fin = $fecha; //fecha = hoy
                $ssucursal = 1;
            } 
            // if ($ssucursal == 1) $ssucursaltxt = "COCHABAMBA";
            // else if ($ssucursal == 2) $ssucursaltxt = "LA PAZ";
            // else if ($ssucursal == 3) $ssucursaltxt = "SANTA CRUZ";
            // else if ($ssucursal == 4) $ssucursaltxt = "TARIJA";
            // else $ssucursaltxt = "TODAS";
            ?>
            <div class="row tableUsers">
                <div class="col-md-12">
                    <div class="panel panel-warning">
                        <div class="panel-heading">
                            <h4 class="panel-title">Lista de facturas <b><?php echo $ssucursaltxt ?></b>
                            ...
                            <div class="panel-heading-btn">
                                <div class="input-group input-daterange">
                                    <form action="?root=facturacionReportes" method="post">
                                        <input id="fechaInicio" required="" type="date" name="inicio" value="<?php echo $Inicio ?>">
                                        <input id="fechaFin" required="" type="date" name="fin" value="<?php echo $Fin ?>">
                                        
                                        <select id="ssucursal" name="ssucursal">
                                            <option value="0" <?php if($ssucursal == "0") echo "selected"; ?>>TODAS</option>
                                            <option value="1" <?php if($ssucursal == "1") echo "selected"; ?>>COCHABAMBA</option>
                                            <option value="2" <?php if($ssucursal == "2") echo "selected"; ?>>LA PAZ</option>
                                            <option value="3" <?php if($ssucursal == "3") echo "selected"; ?>>SANTA CRUZ</option>
                                            <option value="5" <?php if($ssucursal == "5") echo "selected"; ?>>SC TROMPILLO</option>
                                        </select>  
                                        <input type="submit" class="btn btn-xs btn-danger" value="BUSCAR">
                                    </form>

                                </div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                <input type="button" id="imprimirFacturas" class="btn btn-xs btn-danger imprimirFacturas" value="Imprimir Facturas">
                        ...
                        <div class="panel-body">
                            <div class="row">
                                ...
                                <div class="col-xl-3 col-md-6">
                                    ...
                                    FROM
                                        factura
                                    WHERE  siatCodeState = 908 AND ( DATE_FORMAT(dateEmission, '%Y-%m-%d') BETWEEN '$Inicio' AND '$Fin' )") or die(mysqli_error($MySQLi));
                                            else
                                                $queryVentas    =    mysqli_query($MySQLi, "SELECT
                                        SUM(amountTotal) AS amountTotal
                                    FROM
                                        factura
                                    WHERE
                                        branchId = $ssucursal AND siatCodeState = 908 AND ( DATE_FORMAT(dateEmission, '%Y-%m-%d') BETWEEN '$Inicio' AND '$Fin' )") or die(mysqli_error($MySQLi));
                                ...
                                <div class="col-xl-3 col-md-6">
                                    <div class="widget widget-stats bg-success">
                                        ...
                                        FROM
                                        factura
                                    WHERE siatCodeState = 908 AND ( DATE_FORMAT(dateEmission, '%Y-%m-%d') BETWEEN '$Inicio' AND '$Fin' )");
                                            else
                                                $queryEntregadas    =    mysqli_query($MySQLi, "SELECT *
                                        FROM
                                        factura
                                    WHERE
                                        branchId = $ssucursal AND siatCodeState = 908 AND ( DATE_FORMAT(dateEmission, '%Y-%m-%d') BETWEEN '$Inicio' AND '$Fin' )");
                                ...
                                <div class="col-xl-3 col-md-6">
                                    <div class="widget widget-stats bg-gray">
                                        ...
                                        FROM
                                        factura
                                    WHERE siatCodeState = 905 AND ( DATE_FORMAT(dateEmission, '%Y-%m-%d') BETWEEN '$Inicio' AND '$Fin' )");
                                            else
                                                $queryCantidadAnuladas    =    mysqli_query($MySQLi, "SELECT *
                                        FROM
                                        factura
                                    WHERE
                                        branchId = $ssucursal AND siatCodeState = 905 AND ( DATE_FORMAT(dateEmission, '%Y-%m-%d') BETWEEN '$Inicio' AND '$Fin' )");
                                    ...
                                    <div class="widget widget-stats bg-info">
                                        ...
                                        <div class="stats-info">
                                            ...
                                            <p><?php
                                                ...
                                                $QueryFactura = mysqli_query($MySQLi, "SELECT* FROM detailInvoice WHERE branchId=$ssucursal  AND( DATE_FORMAT(dateEmission, '%Y-%m-%d') BETWEEN '$Inicio' AND '$Fin' ) ORDER BY invoiceNumber DESC");
                                                ...
                                                while ($data = mysqli_fetch_assoc($QueryFactura)) {
                                                    ...
                                                    $QueryFacturaCabezera = mysqli_query($MySQLi, "SELECT* FROM factura WHERE branchId=$ssucursal AND invoiceNumber='$nroFactura'  ");
                            ...
                            <table id="data-table-buttons" width="100%" class="table table-striped table-bordered table-td-valign-middle w-100">
                                ...
                                <tbody>
                                    ...
                                    //$QueryFactura = mysqli_query($MySQLi, "SELECT* FROM factura WHERE branchId=1 ORDER BY invoiceNumber DESC");



                                    //         // $QueryFactura    =    mysqli_query($MySQLi,"SELECT* FROM factura WHERE branchId=$branchId ORDER BY invoiceNumber DESC");
                                    ...
                                    //     ");
                                    if ($ssucursal == 0) {
                                        $qryF = "SELECT* FROM detailInvoice WHERE (DATE_FORMAT(dateEmission, '%Y-%m-%d') BETWEEN '$Inicio' AND '$Fin' )  ORDER BY invoiceNumber DESC";
                                        $QueryFactura = mysqli_query($MySQLi, $qryF);
                                        ...
                                        $qryF = "SELECT* FROM detailInvoice WHERE branchId=$ssucursal AND(DATE_FORMAT(dateEmission, '%Y-%m-%d') BETWEEN '$Inicio' AND '$Fin' )  ORDER BY invoiceNumber DESC";
                                        $QueryFactura = mysqli_query($MySQLi, $qryF);
                            ...
                            <input type="hidden" name="q" id="q" value="<?php echo(base64_encode($qryF))?>">
                            
		<a href="#" onclick="return theFunction();">Ver todo</a>
<script type="text/javascript">
  function theFunction () {
        let inicio = $("#fechaInicio").val();
        let fin = $("#fechaFin").val();
        let sucursal = $("#ssucursal").val();
        let url = './Paginas/factura_pdf2.php?inicio=' + inicio + '&fin=' + fin + '&sucursal=' + sucursal;
